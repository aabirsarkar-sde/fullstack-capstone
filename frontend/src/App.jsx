...
export default function App() {
  ...
  const handleSignup = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://fullstack-capstone-rf3g.onrender.com/api/signup', form);
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://fullstack-capstone-rf3g.onrender.com/api/login', form);
      localStorage.setItem('token', response.data.token);
      setView('users');
      fetchUsers();
    } catch (error) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await axios.get('https://fullstack-capstone-rf3g.onrender.com/api/users', {
